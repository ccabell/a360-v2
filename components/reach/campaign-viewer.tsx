...
const TYPE_META: Record<
  string,
  { label: string; badge: string; accent: string; blurb: string }
> = {
  BOOKING_FACILITATION: {
    ...
    blurb: "Close the scheduling gap — patient committed but not yet booked.",
  },
  VALUE_REINFORCEMENT: {
    ...
    blurb: "Reduce friction and address the specific hesitation raised.",
  },
  MARKETING_OPPORTUNITY: {
    ...
    blurb: "Post-care follow-through and nurture the future treatment interest.",
  },
};
  ...
  return (
    <div className="relative pl-8">
      ...
      <div className="absolute left-[11px] top-1 flex flex-col items-center">
        <span className="flex h-6 w-6 items-center justify-center rounded-full border-2 border-primary/40 bg-background text-[10px] font-bold text-primary">
          {index + 1}
        </span>
      </div>
      {!isLast && (
        <div className="absolute left-[22px] top-8 bottom-[-20px] w-px bg-border" />
      )}

      <div className="rounded-xl border border-border bg-background/60 overflow-hidden mb-5">
        {/* header */}
        ...
        {signals.length > 0 && (
          <div className="border-t border-border/60 px-4 py-2">
            ...
            >
              ...
              <Link2 className="h-3 w-3" />
              {signals.length} personalization signal
              {signals.length !== 1 ? "s" : ""}
            </button>
            {signalsOpen && (
              <div className="mt-2 space-y-1.5">
                {signals.map((s, i) => (
                  ...
                  >
                    ...
                    <p className="mt-0.5 font-mono text-[10px] text-muted-foreground/60 break-all">
                      {s.key}
                    </p>
                  </div>
                ))}
          ...
          <div className="border-t border-border/60 bg-muted/10 px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                onClick={submit}
                disabled={regenLoading || !instruction.trim()}
              >
                {regenLoading ? (
...
export function CampaignViewer({
  ...
  const emailSignals = (email: CampaignEmail): PersonalizationSignal[] => {
    const refs = email.personalization_refs ?? [];
    return refs.map(
      (r) => signalByKey.get(r) ?? { key: r, value: "" },
    );
  };
  ...
  const handleRegenerate = async (index: number, instruction: string) => {
    ...
    try {
      ...
      const raw = await runComposerToText({
        agentId,
        patientId,
        userMessage: prompt,
      });
      ...
      const updated: CampaignEmail = {
        ...
        preview_text:
          typeof src.preview_text === "string"
            ? src.preview_text
            : email.preview_text,
        body_markdown:
          typeof src.body_markdown === "string"
            ? src.body_markdown
            : email.body_markdown,
        cta,
      ...
      // Merge any newly introduced signals (dedupe by key).
      const addedRaw = Array.isArray(obj.added_signals)
        ? (obj.added_signals as unknown[])
        : [];
        ...
        if (a && typeof a === "object") {
          ...
          if (typeof s.key === "string" && !keys.has(s.key)) {
            nextSignals.push({
              ...
              used_in: Array.isArray(s.used_in)
                ? (s.used_in as string[])
                : undefined,
            });
            keys.add(s.key);
      ...
      onCampaignChange({
        ...campaign,
        email_sequence: nextSeq,
        personalization_signals: nextSignals,
      });
    } catch (e) {
      setRegenError(
    ...
    <TooltipProvider delay={80}>
      <div className="mt-6 rounded-2xl border border-border bg-card/40 overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-3 border-b border-border bg-muted/20 px-5 py-3">
          ...
          <div className="flex-1 min-w-[180px]">
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              ...
              {(campaign.personalization_signals ?? []).length} personalization
              signals
            </p>
          </div>
          <Button
        ...
        <div className="px-5 py-4 space-y-4">
          {/* classification / why this campaign */}
                ...
                <div className="flex items-start gap-2 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-xs text-red-800 dark:border-red-800/60 dark:bg-red-950/30 dark:text-red-300">
                  ...
                  <div>
                    <p className="font-semibold">
                      {violated.length} guardrail
                      {violated.length !== 1 ? "s" : ""} violated
                    </p>
              ...
              {crmOpen && (
                <div className="mt-3">
                  <Tabs defaultValue="targeted">
                    ...
                    <TabsContent value="targeted" className="mt-3">
                      <pre className="overflow-x-auto rounded-lg border border-border bg-muted/30 p-3 text-xs font-mono text-foreground/80">
                        {JSON.stringify(
