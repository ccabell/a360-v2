...
import type { ProductCard, ProductPairing, ProductEvidence } from "@/lib/types/products";
...
export function ProductsBrowser({
  ...
  const [selectedId, setSelectedId] = useState<string | null>(() => {
    const linked = initialSelectedId ? products.find((p) => p.id === initialSelectedId) : undefined;
    return (linked ?? products[0])?.id ?? null;
  });
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-border bg-card p-10 text-center">
        <AlertTriangle className="mx-auto h-6 w-6" style={{ color: ACCENT }} />
        <p className="mt-3 text-sm font-medium text-foreground">Couldn&apos;t load the product catalog</p>
        <p className="mt-1 text-xs text-muted-foreground">It&apos;s temporarily unavailable — please refresh to try again.</p>
        <p className="mt-3 text-[11px] text-muted-foreground/70">{error}</p>
      </div>
    );
  }
        ...
        <div className="max-h-[74vh] overflow-y-auto pr-1 space-y-4">
          ...
          {grouped.length === 0 && (
            <p className="px-1 text-sm text-muted-foreground">
              {products.length === 0 ? "No products available yet." : "No matches."}
            </p>
          )}
        </div>
      </div>
          ...
          {p.evidence.length > 0 ? (
            <>
              <p className="mb-3 text-xs text-muted-foreground">
                {p.evidence.length} clinical {p.evidence.length === 1 ? "citation" : "citations"} — FDA labels, PubMed studies and manufacturer IFUs backing the claims above.
              </p>
...
function EvidenceItem({ evidence: e }: { evidence: ProductEvidence }) {
  const title = e.source_reference || e.claim_text || e.field_name || "Source";
  const body = e.snippet && e.snippet !== title ? e.snippet : e.source_reference ? e.claim_text : null;
  const ref = e.pmid ? `PMID ${e.pmid}` : e.doi ? `DOI ${e.doi}` : null;

  return (
    <div className="rounded-xl border border-border p-3">
      <div className="mb-1 flex items-center gap-2 flex-wrap">
        <span className="rounded-md px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide" style={{ background: "rgba(245,166,35,0.14)", color: "#9a6207" }}>
          {SOURCE_LABELS[e.source] ?? e.source}
        </span>
        {ref && <span className="text-[11px] text-muted-foreground">{ref}</span>}
      </div>
      <p className="text-sm font-medium text-foreground">{title}</p>
      {body && <p className="mt-0.5 text-sm text-foreground/70 line-clamp-3">{body}</p>}
      {e.url && (
        <a href={e.url} target="_blank" rel="noreferrer" className="mt-1.5 inline-flex items-center gap-1 text-xs font-medium hover:underline" style={{ color: "#9a6207" }}>
